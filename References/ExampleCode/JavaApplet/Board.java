...
public class Board {
  ...
  public void Check () {
    if (eval() >= 1) bC.curString="OK you win";
  ...
  public int minimax(int Level, int alfa, int beta){
    ...
    if (thisBoard[0]==0) {
       ...
       if (((this.pieceToPut & thisBoard[1] & thisBoard[2] & thisBoard[3])!=0) ||
           ...
           evalt += (System.currentTimeMillis() - t2);
           return 1;
        }       
    }
    if (thisBoard[1]==0) {
          ...
          for(int k = 0;(k < 16); k++){
              if (piecesAvailable[k]){
                     ...
                     if (Level == 1){
                        ...
                        long i3 = (System.currentTimeMillis() - t2);
                        evalt += i3;
                        /* END OF EVAL */
  ...
  public void Think () {
     ...
     if (pieceToPut == EMPTY){
       ...
       if (nrMovesDone == 15) {
          ...
          System.out.println("We spent " + evalt + " millisecs or " + evalt/1000 + " secs in eval");
